		...
		<fieldset>
			...
			<details v-for="(v, i) in props.component.dataSources" :key="v.name" open class="details">
				...
				<fieldset>
					...
					<FormItem
						v-if="v.request.body.type === 'form-data'"
						for="setter-data-source-request-form-data-params"
					>
					...
					>
						<table id="setter-data-source-request-x-www-form-urlencoded-params">
							...
							<tbody>
								<tr
									v-for="(v2, i2) in [
										...v.request.body.xWwwFormUrlencodedParams,
										{ key: '', value: '' },
									]"
									:key="i2"
								>
					...
					<FormItem label="响应类型" for="setter-data-source-response-type">
						<select
							...
							@input="
								v.responseType = ($event.target as HTMLSelectElement).value as DataSourceResponseType
							"
						>
							<option value="none">none</option>
							<option value="Text">Text</option>
					...
					<FormItem label="响应数据" for="setter-data-source-response-data">
						<textarea
							id="setter-data-source-response-data"
							readonly
							:value="getResponseDataShow(v)"
						></textarea>
					</FormItem>
					<FormItem
						label="执行动作前"
